...
function MiniBar({ hours, max = OVERLOAD_THRESHOLD }: { hours: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-sm font-medium tabular-nums" style={{ color: statusHex[status], minWidth: 32 }}>
        {hours}h
      </span>
...
type LoadFilter = "all" | "under20" | "mid" | "over35";
...
export default function AllocationsPageClient({ initialAllocations, initialInterns, initialProjects }: Props) {
  ...
  const filtered = allocations.filter((a) => {
    ...
    if (loadFilter === "mid" && (load < 20 || load > 35)) return false;
    ...
    <>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <div className="flex rounded-lg border border-card-border overflow-hidden text-sm">
          {(["all", "under20", "mid", "over35"] as const).map((v) => (
            <button
              key={v}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((a, i) => {
              ...
              const loadStatus = getLoadStatus(internLoad);
              ...
              return (
                <Fragment key={a.id}>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div
                          ...
                          style={{ backgroundColor: statusBgHex[loadStatus], color: statusHex[loadStatus] }}
                        >
                          {initials(name)}
                        </div>
                    ...
                    <td className="px-4 py-3 text-slate-500">{formatDateShort(a.start_date)}</td>
                    <td className="px-4 py-3 text-slate-500">{formatDateShort(a.end_date)}</td>
                        ...
                        <div className="flex items-start gap-8">
                          ...
                          <div className="border-l border-card-border pl-8">
                            ...
                            <div className="flex items-center gap-3">
                              <span className="text-2xl font-bold" style={{ color: statusHex[loadStatus] }}>
                                {internLoad}h
                              </span>
                              <span
