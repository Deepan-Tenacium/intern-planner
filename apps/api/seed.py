...
users_data = [
    {
        "email": "manager@tenacium.com",
        "password": "manager123",
        "name": "Jahnavi",
        "role": "manager",
    },
    {
        "email": "intern@tenacium.com",
        "password": "intern123",
        "name": "Deepan",
        "role": "intern",
    },
...
for ud in users_data:
    existing = db.execute(
        select(models.User).where(models.User.email == ud["email"])
    ).scalar_one_or_none()
    if not existing:
        db.add(models.User(
            email=ud["email"],
...
# --- Skills / Projects / Interns / Allocations (skip if already seeded) ---
if db.execute(select(models.Skill)).first() is None:
    skills_data = [
        ("Python", "backend"),
        ("TypeScript", "frontend"),
        ("React", "frontend"),
...
db.commit()
db.close()
print("Seeded: 2 users, 8 interns, 8 skills, 5 projects, 10 allocations.")
